...
// ── Shared staleness gate for the 30-min cron, trade execution, and any
// route that reads price_cache. Whoever asks first in a given window pays
// for the CoinGecko call; everyone else (cron ticks, trades, other requests)
// rides on the cache until it's stale again. Always returns a complete
// symbol->price map, whether or not a live fetch actually happened, so
// callers never need to special-case a skipped refresh.
// requiredSymbols: if the caller needs specific symbols that have NEVER
// been cached (e.g. a coin just added to a class), the global "most recent
// update" check alone wouldn't catch that — pass the symbols this specific
// call needs and a refresh is forced if any of them is missing entirely.
export async function refreshPricesIfStale(maxAgeMs = REFRESH_STALE_MS, requiredSymbols = []) {
  const { data: latest } = await db.from('price_cache')
    .select('updated_at').order('updated_at', { ascending: false }).limit(1).single();
  let isStale = !latest || (Date.now() - new Date(latest.updated_at).getTime()) > maxAgeMs;

  if (!isStale && requiredSymbols.length) {
    const { data: haveRows } = await db.from('price_cache').select('symbol').in('symbol', requiredSymbols);
    const have = new Set((haveRows || []).map(r => r.symbol));
    if (requiredSymbols.some(s => !have.has(s))) isStale = true;
  }
