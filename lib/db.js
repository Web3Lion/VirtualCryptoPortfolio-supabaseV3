...
export async function getMarketStatus(classId) {
  ...
  if (classId) {
    ...
    if (cls) {
      seedMoney = parseFloat(cls.seed_money);
      const parsedFee = parseFloat(cls.trade_fee);
      if (!isNaN(parsedFee)) feeRate = parsedFee;
    }
  }
  if (cfg.TRADE_FEE_OVERRIDE) {
    const override = parseFloat(cfg.TRADE_FEE_OVERRIDE);
    if (!isNaN(override)) feeRate = override;
  }
