...
export async function runBot({ classId, config }) {
  ...
  for (const h of holdings) {
    ...
    else if (strategy === 'momentum'   && ch24 < -buyThr)       { sell = true; reason = `momentum_exit 24h=${ch24.toFixed(1)}%`; }
    else if (strategy === 'trend'      && ch7d < 0)             { sell = true; reason = `trend_exit 7d=${ch7d.toFixed(1)}%`; }
