...
export async function runBot({ classId, config }) {
  ...
  for (const h of holdings) {
    ...
    if (sell) {
      const r = await executeTrade({ studentId: botId, classId, action: 'SELL', coin: h.coin, amountType: 'Shares', amount: parseFloat(h.quantity), knownPrice: cur, reasoning: `🤖 Bot (${strategy}): ${reason}` });
  ...
  if (heldCoins.size < maxPos) {
    ...
    if (tradeAmt >= 10) {
      ...
      if (target) {
        const r = await executeTrade({ studentId: botId, classId, action: 'BUY', coin: target.symbol, amountType: 'Dollar Amount', amount: tradeAmt, knownPrice: parseFloat(target.price), reasoning: `🤖 Bot (${strategy}): 24h=${parseFloat(target.change_24h||0).toFixed(1)}% 7d=${parseFloat(target.change_7d||0).toFixed(1)}%` });
