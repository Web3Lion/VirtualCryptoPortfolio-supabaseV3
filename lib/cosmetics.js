...
export function applyCosmetic(themeId) {
  ...
  if (!theme) {
    // Clear any previously applied cosmetic inline vars so the base theme takes over
    const allVarKeys = Object.values(COSMETIC_THEMES).flatMap(t => Object.keys(t.vars));
    [...new Set(allVarKeys)].forEach(k => root.style.removeProperty(k));
    return;
  }
