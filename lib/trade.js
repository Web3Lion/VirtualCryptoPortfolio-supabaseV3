...
export async function executeTrade({ studentId, classId, action, coin, amountType, amount, reasoning = '', leverageMultiplier = 1 }) {
  ...
  let quantity, grossValue, fee, newCash, newFees, recordedAction;
  ...
  await Promise.all([
    ...
    db.from('trades').insert({ student_id: studentId, class_id: classId, action: recordedAction, coin, quantity, price, gross_value: grossValue, fee, cash_after: newCash, reasoning }),
    db.from('snapshots').insert({ student_id: studentId, class_id: classId, total_value: newCash, cash: newCash, snapshot_type: 'intraday' }),
  ]);

  return { success: true, message: `✅ ${recordedAction} ${quantity.toFixed(6)} ${coin} @ ${fmtUSD(price)}`, price, quantity, fee, newCash, action: recordedAction };
}
