...
export async function executeTrade({ studentId, classId, action, coin, amountType, amount, reasoning = '', leverageMultiplier = 1, knownPrice = null }) {
  ...
  if (action === 'BUY') {
    ...
    if (currentQty < -0.000001) {
      ...
      if (Math.abs(remaining) < 0.000001) {
        ({ error: holdingsErr } = await db.from('holdings').delete().eq('student_id', studentId).eq('class_id', classId).eq('coin', coin));
      } else {
        ({ error: holdingsErr } = await db.from('holdings').update({ quantity: remaining, updated_at: new Date().toISOString() }).eq('student_id', studentId).eq('class_id', classId).eq('coin', coin));
      }
      if (holdingsErr) { console.error('[executeTrade] COVER holdings write failed:', holdingsErr); return { success: false, error: `❌ Could not update holdings: ${holdingsErr.message}` }; }
    } else {
      ...
      if (holding && currentQty > 0) {
        ...
        ({ error: holdingsErr } = await db.from('holdings').update({ quantity: newQty, avg_buy_price: newAvg, margin_borrowed: newBorrowed, updated_at: new Date().toISOString() }).eq('student_id', studentId).eq('class_id', classId).eq('coin', coin));
      } else {
        // Upsert (not a bare insert): if a stale/leftover row for this
        // student+class+coin already exists (e.g. from a race with another
        // write to the same coin, like a staking payout landing at the same
        // moment), a plain insert silently violates the unique constraint
        // and is lost — the trade still records, the holding never appears.
        ({ error: holdingsErr } = await db.from('holdings').upsert(
          { student_id: studentId, class_id: classId, coin, quantity, avg_buy_price: price, margin_borrowed: borrowed, updated_at: new Date().toISOString() },
          { onConflict: 'student_id,class_id,coin' }
        ));
      }
      if (holdingsErr) { console.error('[executeTrade] BUY holdings write failed:', holdingsErr); return { success: false, error: `❌ Could not update holdings: ${holdingsErr.message}` }; }
    }
  } else if (action === 'SELL') {
    ...
    if (remaining <= 0.000001) {
      ({ error: sellHoldingsErr } = await db.from('holdings').delete().eq('student_id', studentId).eq('class_id', classId).eq('coin', coin));
      if (!sellHoldingsErr) {
      ...
      ({ error: sellHoldingsErr } = await db.from('holdings').update({ quantity: remaining, margin_borrowed: remainingBorrowed, updated_at: new Date().toISOString() }).eq('student_id', studentId).eq('class_id', classId).eq('coin', coin));
    }
    if (sellHoldingsErr) { console.error('[executeTrade] SELL holdings write failed:', sellHoldingsErr); return { success: false, error: `❌ Could not update holdings: ${sellHoldingsErr.message}` }; }
    ...
    if (holding && currentQty < -0.000001) {
      ...
      ({ error: shortHoldingsErr } = await db.from('holdings').update({ quantity: newQty, avg_buy_price: newAvg, updated_at: new Date().toISOString() }).eq('student_id', studentId).eq('class_id', classId).eq('coin', coin));
    } else {
      // Upsert as a safety net against a stale/leftover row for this coin —
      // see the BUY branch above for why a bare insert can silently fail.
      ({ error: shortHoldingsErr } = await db.from('holdings').upsert(
        { student_id: studentId, class_id: classId, coin, quantity: -quantity, avg_buy_price: price, margin_borrowed: 0, updated_at: new Date().toISOString() },
        { onConflict: 'student_id,class_id,coin' }
      ));
    }
    if (shortHoldingsErr) { console.error('[executeTrade] SHORT holdings write failed:', shortHoldingsErr); return { success: false, error: `❌ Could not update holdings: ${shortHoldingsErr.message}` }; }
  } else {
  ...
  const [portfolioWrite, tradeWrite] = await Promise.all([
  ...
  if (portfolioWrite.error) console.error('[executeTrade] portfolio cash update failed:', portfolioWrite.error);
  if (tradeWrite.error) console.error('[executeTrade] trade history insert failed:', tradeWrite.error);
...
export async function executeSellAll({ studentId, classId }) {
  ...
  writes.forEach(w => { if (w.error) console.error('[executeSellAll] write failed:', w.error); });
