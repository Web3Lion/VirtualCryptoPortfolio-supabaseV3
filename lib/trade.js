...
export async function executeTrade({ studentId, classId, action, coin, amountType, amount, reasoning = '', leverageMultiplier = 1 }) {
  ...
  // Shares one 30-min staleness gate with the cron: if prices were refreshed
  // recently (by the cron or another trade), this just reads price_cache —
  // no CoinGecko call. Otherwise it does one live batch refresh for every
  // coin, not just this one, since the API call costs the same either way.
  let rawPrice = null;
  try {
    const { priceMap } = await refreshPricesIfStale();
    rawPrice = priceMap[coin] || null;
  } catch {}
  if (!rawPrice) {
...
export async function executeSellAll({ studentId, classId }) {
  ...
  const { priceMap: freshPrices } = await refreshPricesIfStale();
  const prices = holdings.map(h => freshPrices[h.coin] || null);
