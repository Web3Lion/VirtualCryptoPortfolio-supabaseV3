...
export default function NewsPage() {
  ...
  const [scenarioNews, setScenarioNews] = useState(null);
  ...
  useEffect(()=>{
    if(status==='authenticated'){
      Promise.all([
        ...
        fetch('/api/news/scenario').then(r=>r.ok?r.json():null).catch(()=>null),
      ]).then(([p,a,s])=>{
        setPushed(Array.isArray(p)?p:[]);
        setArticles(Array.isArray(a)?a:[]);
        if(s?.active && s.headlines?.length) setScenarioNews(s);
    ...
    <>
      ...
      <div className="page">
        ...
        {loading ? (
          ...
          <>
            {scenarioNews && scenarioNews.headlines.length > 0 && (
              <>
                <div style={{background:'rgba(167,139,250,.1)',border:'1px solid rgba(167,139,250,.35)',borderRadius:16,padding:'16px 20px',marginBottom:24}}>
                  <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
                    <span style={{fontSize:20}}>📅</span>
                    <div>
                      <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:15,color:'#a78bfa'}}>Historical Headlines</div>
                      <div style={{fontSize:11,color:'var(--muted)'}}>AI-generated context for scenario: {scenarioNews.label}</div>
                    </div>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:10}}>
                    {scenarioNews.headlines.map((h, i) => (
                      <div key={i} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'12px 16px',display:'flex',alignItems:'flex-start',gap:12}}>
                        <span style={{fontSize:22,flexShrink:0,marginTop:1}}>{h.emoji}</span>
                        <div style={{flex:1}}>
                          <div style={{fontSize:13,fontWeight:600,color:'var(--text)',marginBottom:3}}>{h.title}</div>
                          <div style={{fontSize:11,color:'var(--muted)'}}>{h.summary}</div>
                        </div>
                        <span style={{fontSize:9,padding:'2px 8px',borderRadius:6,fontWeight:700,letterSpacing:1,flexShrink:0,marginTop:2,
                          background:h.sentiment==='bullish'?'rgba(0,229,160,.12)':h.sentiment==='bearish'?'rgba(244,63,94,.1)':'rgba(71,85,105,.2)',
                          color:h.sentiment==='bullish'?'var(--up)':h.sentiment==='bearish'?'var(--down)':'var(--muted)'}}>
                          {(h.sentiment||'neutral').toUpperCase()}
                        </span>
                      </div>
                    ))}
                  </div>
                  <div style={{fontSize:10,color:'var(--muted)',marginTop:10,textAlign:'right'}}>Generated by Claude · Educational context only</div>
                </div>
              </>
            )}
